...
a string: C<Time: (xx:yy.zz)>. The script can catch this time and collect all
...
by this script with C<-l> option.
...
strings defined there too. All tokens with C<%> sign are replaced by their
...
stop.  So, if you define C<%blenderPath> and C<%blenderExe> like this:
...
I<epsilon> - the change which tells us, how much the average is changed between
...
=head3 Output of the blender command
